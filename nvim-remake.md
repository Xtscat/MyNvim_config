...
- **调色板文件 `modules/ui/themes/onehalf_latte.lua`**：色相取自 One Half Light 自己的定义（VSCode 的 `OneHalfLight.tmTheme` + `vim/colors/onehalflight.vim`），刺眼的那三项按实测调过：强调色 饱和度 ×0.95 且亮度 ×0.96（比原版深、比纯降饱和饱）、搜索高亮从原版“白底士黄块”改成 25/40/60% 的淡黄、面板与 chrome 保持明确步进（浮窗/picker/dock = `mantle #dee1e5`，tabline/分隔线 = `crust #d4d8de`）。**底色回到 `#fafafa`**（issue #4：和 edge 一致，用户试用后确认更喜欢），代价是各步进变强（浮窗 7.8%、chrome 10.6%、正文 8.9:1），换来的是搜索窗/dock 与代码底更好区分。浮窗里的次级文字（picker 的路径/注释/行号列、补全菜单的描述）用专门的 `DIM #767a81`（在浮窗底上 3.3:1）：catppuccin 原本把它们 link 到 `NonText`/`Comment`/`LineNr`，在白底浮窗上只有 1.4–2.2:1，读不清（issue #5）。catppuccin 有 26 个色槽而 One Half 只有 6 个色相，所以色相复用是故意的（变量/标签/property 红、函数蓝、关键字紫、类型与数字琥珀、字符串绿、转义青、注释 `#97989e` 斜体、参数与运算符=正文色、选区 `#bfceff`）。
- **运行时切主题会留下“僵尸组”，必须自己补**：一批插件在**创建窗口/首次渲染时**从当前主题**算一次颜色**存成具体值（不是 link），而不是每次 `ColorScheme` 重算；切到浅色后这些组会保留深色主题的值，表现为 UI 里的**黑块**（实测：`NeoTreeEndOfBuffer` 在 neotree dock 的空白行 = onedark 的 `#21252b`；还有 `Terminal`、`StatusLineTerm(NC)`、`Outline*`、`GitConflict*`、`GitSignsDiffStaged`）。这些组大多不属于 catppuccin 的适配表，所以写在我们自己的 `M.highlights` 里（它在 catppuccin 编译结果里、晚于插件的 `ColorScheme` 回调，所以能赢）。
