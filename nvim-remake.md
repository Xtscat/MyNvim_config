...
- **调色板文件 `modules/ui/themes/onehalf_latte.lua`**：色相取自 One Half Light 自己的定义（VSCode 的 `OneHalfLight.tmTheme` + `vim/colors/onehalflight.vim`），刺眼的那三项按实测调过：底色 `#fafafa`→`#f1f2f4`（发光量 −6.3%）、正文对比 10.1:1→**8.32:1**、面板/边框步进 0%→**5.4%/8.3%**、强调色 饱和度 ×0.95 且亮度 ×0.96（比原版深、比纯降饱和饱）、搜索高亮从原版“白底士黄块”改成 25/40/60% 的淡黄。catppuccin 有 26 个色槽而 One Half 只有 6 个色相，所以色相复用是故意的（变量/标签/property 红、函数蓝、关键字紫、类型与数字琥珀、字符串绿、转义青、注释 `#97989e` 斜体、参数与运算符=正文色、选区 `#bfceff`）；补全菜单（`Pmenu`/`BlinkCmpLabel`）也用正文色而不是 catppuccin 默认的注释灰。
